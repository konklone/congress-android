...
public class NotificationService extends WakefulIntentService {
	...
	protected void doWakefulWork(Intent intent) {
		...
		do {
			...
			List<String> unseenIds = new ArrayList<>();
			...
			if (alwaysNotify || (results > 0)) {
				...
				notifyManager.notify(
					(subscription.id + subscription.notificationClass).hashCode(),
						getNotification(subscription.notificationClass,
	...
	private Notification getNotification(String notificationClass, String ticker, String title,
										 String message, Intent intent, List<String> unseenIds, Uri uri) {
		...
		String ringtone = PreferenceManager.getDefaultSharedPreferences(this).getString(NotificationSettings
				.KEY_NOTIFY_RINGTONE, NotificationSettings.DEFAULT_NOTIFY_RINGTONE);
		if (ringtone != null)
			notification.sound = Uri.parse(ringtone);

		// Vibrate unless user disabled it
		boolean vibration = PreferenceManager.getDefaultSharedPreferences(this).getBoolean(NotificationSettings
				.KEY_NOTIFY_VIBRATION, NotificationSettings.DEFAULT_NOTIFY_VIBRATION);
		if (vibration)
